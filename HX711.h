...
private:
  ...
  long values[BUFFERSIZE_MAX];
  uint8_t valuesBufferSize;
  uint8_t index;
...
public:
  ...
  // channel A, 32 for channel B
  // gain: 128 or 64 for channel A; channel B works with 32 gain factor only
  HX711(uint8_t newValuesBufferSize,
        byte    dout,
        byte    pd_sck,
        byte    gain = 128);
  ...
  // output pin DOUT is high. Serial clock
  // input PD_SCK should be low. When DOUT goes to low, it indicates data is
  // ready for retrieval.
  bool is_ready();
